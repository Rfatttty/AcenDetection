...
import '../back_camera/rightface_cameraPage.dart';
import 'rightface_cameraPage.dart';

class middleface_frontcameraPage extends StatefulWidget {
  const middleface_frontcameraPage(
      {Key? key,
  ...
  @override
  middleface_frontcameraPageState createState() =>
      middleface_frontcameraPageState();
}

class middleface_frontcameraPageState
    extends State<middleface_frontcameraPage> {
  ...
  Widget build(BuildContext context) {
    final middleface_camerabody = Transform.scale(
      scale: 1.0,
      child: SafeArea(
        child: Stack(
          children: [
            ...
            children: [
                  ...
                  child: FloatingActionButton(
                    onPressed: () async {
                      try {
                        ...
                        final middlefaceimage = await _controller.takePicture();
                        print(stopwatch.elapsedMilliseconds);

                        await _controller.setFocusMode(FocusMode.auto);
                        await _controller.setExposureMode(ExposureMode.auto);

                        middlefaceimage.saveTo(path);
                        print(stopwatch.elapsedMilliseconds);
                        ...
                        Navigator.of(context).push(
                          MaterialPageRoute(
                            builder: (_) => rightface_frontcameraPage(
                              cameras: widget.cameras,
                              username: widget.username,
                              recordname: widget.recordname,
                              image1: widget.image1,
                              image2: middlefaceimage,
                            ),
                          ),
                        );
                        print(stopwatch.elapsedMilliseconds);
              ...
              FloatingActionButton(
                onPressed: () async {
                  Navigator.of(context).push(
                    MaterialPageRoute(
                      builder: (context) => middleface_backcameraPage(
                        cameras: widget.cameras,
                        username: widget.username,
                        recordname: widget.recordname,
                        image1: widget.image1,
    ...
    return WillPopScope(
      ...
      child: Scaffold(
        ...
        body: Stack(
          children: [middleface_camerabody, painter, frontbackChange_btn],
        ),
      ),
    );
  }
